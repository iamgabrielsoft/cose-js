...
class Common {
  ...
  constructor(){
    ...
    'alg': (value) => {
      if (!(Common.AlgToTags[value])) {
        throw new Error('Unknown \'alg\' parameter, ' + value);
      }
      return Common.AlgToTags[value];
    }
   ...
   this.KeyTranslators = {
      'kty': (value) => {
        if (!(Common.KeyTypes[value])) {
          throw new Error('Unknown \'kty\' parameter, ' + value);
        }
        return Common.KeyTypes[value];
      },

      'crv': (value) => {
        if (!(Common.KeyCrv[value])) {
          throw new Error('Unknown \'crv\' parameter, ' + value);
        }
        return Common.KeyCrv[value];
      }
   }
  }
  ...
  TranslateHeaders(header){
    const result = new Map()
    for(const param in header){
      if(!Common.HeaderParameters[param]) {
        throw new Error('Unknown parameter, \'' + param + '\'');
      }

      let value = header[param];
      if(this.Translators[param]){
        value = this.Translators[param](header[param]); 
      }

      if(value !== undefined && value !== null){
        result.set(Common.HeaderParameters[param], value); 
      }
    }
  ...
  TranslateKey(key){
    const result = new Map() //map for getting distinct value
    for(const param in key){
      if(Common.KeyParameters[param]){
        throw new Error('Unknown parameter, \'' + param + '\'')
      }

      let value = key[param]; 
      
      if(Common.KeyTranslators[param]){
        value = Common.KeyTranslators[param](value)
      }

      result.set(Common.KeyParameters[param], value); 
